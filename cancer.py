...
def indexClass(Table):
    ...
    for i in range(len(Table[0]) - 1):
        for j in range(len(Table)):
            # print(Table[j][i])
            if Table[j][len(Table[0]) - 1] == 'M':
                # print('x')
                temp1.append(float(Table[j][i]))
            elif Table[j][len(Table[0]) - 1] == 'B':
                # print('a')
                temp2.append(float(Table[j][i]))
            elif Table[j][len(Table[0]) - 1] == 'N':
                # print('b')
                temp3.append(float(Table[j][i]))
        AtrSetosa.append(temp1);
        Atrvirginica.append(temp2);
        Atrversicolor.append(temp3)
        temp1 = [];
        temp2 = [];
        temp3 = []
    '''
...
def NaiveBayes(Table, w, x, y, z, DeviasiMean, pxyz):
    ...
    for i in range(len(Table[0]) - 2):
        norm1 = (1 / (2 * math.pi * (DeviasiMean[1][i][0] ** 2)) ** 1 / 2) * math.exp(1) ** (
                (-1 * (w - DeviasiMean[0][i][0]) ** 2) / (2 * DeviasiMean[1][i][0] ** 2))
        norm2 = (1 / (2 * math.pi * (DeviasiMean[1][i][1] ** 2)) ** 1 / 2) * math.exp(1) ** (
                (-1 * (x - DeviasiMean[0][i][1]) ** 2) / (2 * DeviasiMean[1][i][1] ** 2))
        norm3 = (1 / (2 * math.pi * (DeviasiMean[1][i][2] ** 2)) ** 1 / 2) * math.exp(1) ** (
                (-1 * (y - DeviasiMean[0][i][2]) ** 2) / (2 * DeviasiMean[1][i][2] ** 2))
        norm4 = (1 / (2 * math.pi * (DeviasiMean[1][i][3] ** 2)) ** 1 / 2) * math.exp(1) ** (
                (-1 * (z - DeviasiMean[0][i][3]) ** 2) / (2 * DeviasiMean[1][i][3] ** 2))
...
def forcasting():
    ...
    # print(c[1])
    px = pxyz(a)
    w = float(form.getvalue("radius_mean"))
    x = float(form.getvalue("texture_mean"))
    y = float(form.getvalue("perimeter_mean"))
    z = float(form.getvalue("area_mean"))
    prediction = NaiveBayes(a, w, x, y, z, c, px)
    print(prediction)
